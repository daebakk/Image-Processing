...
def Encoding(src, n=8):
    ...
    #subtract 128
    blocks -= 128
    print('subtract blocks shape : {}'.format(blocks.shape))
    #DCT
    blocks_dct = []
    for block in blocks:
        blocks_dct.append(DCT(block, n=n))
    blocks_dct = np.array(blocks_dct)
    print('DCT shape : {}'.format(blocks_dct.shape))

    #Quantization + thresholding
    Q = Quantization_Luminance()
    QnT = np.round(blocks_dct / Q)

    print('QnT shape : {}'.format(QnT.shape))
    # zigzag scanning
    # zz는 4096개의 원소를 갖고 각 원소는 8 x 8 size를 갖는 block이다.
    zz = []
    for i in range(len(QnT)):
        zz.append(my_zigzag_scanning(QnT[i]))
    print('zz len : {}'.format(len(zz)))
    print('zz[0] : {}'.format(zz[0]))
    return zz, src.shape
